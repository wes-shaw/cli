...
describe('app config link command', () => {
  test('accepts --client-id with --config to link a specific app to a specific config file', async () => {
    await inTemporaryDirectory(async (tmp) => {
      ...
      await ConfigLink.run(['--path', tmp, '--client-id', 'api-key', '--config', 'staging'], import.meta.url)
